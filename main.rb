...
class Mastermind
  ...
  def breaker_mode
    ...
    12.times do
      ...
      next unless undeciphered_code == guess
    ...
    puts 'You win, CODEBREAKER failed to figure out your code!' if @codebreaker_win == false
  ...
  def guess_mode
    ...
    undeciphered_code = @computer.generate_code
    ...
    puts "You lose, you failed to figure out the CODEMAKER's code!" if @codebreaker_win == false
  ...
  def compare_guess(undeciphered_code, guess)
    ...
    modified_guess_array = guess.reject.with_index { |_e, i| positional_match[i] }
    modified_code_array = undeciphered_code.reject.with_index { |_e, i| positional_match[i] }
...
class Computer
  ...
  def break_code(undeciphered_code)
    ...
    array.each_with_index do |e, i|
      @modified_array[i] = undeciphered_code[i] if e == undeciphered_code[i]
    end
    @modified_array
  end
end
...
class Player
  ...
  def input_mode
    loop do
      ...
      break input_value if [1, 2].include?(input_value)
